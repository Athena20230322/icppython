...
def process_iyugo_barcode_generation():
    ...
    print(f"[*] 啟動測試開發工具：產出 Token 並執行 P0018 確認支付...")
    ...
    for idx, line in enumerate(lines[1:], 1):
        ...
        try:
            ...
            for i in range(count_val):
                ...
                if trade_token:
                    print(f"    └─ [{i + 1}] Token 產生成功: {trade_token[:10]}...")

                    # (B) 執行 P0018 DoQRPayment (在此新增)
                    pay_payload = {
                        "Amount": "5",
                        "MerchantID": "10000236",
                        "MerchantTradeNo": trade_no,
                    ...
                    print(f"    └─ [{i + 1}] 執行 P0018 DoQRPayment...")
                    res_do_pay = client.call_api(client.payment_url, "app/Payment/DoQRPayment", pay_payload)

                    if res_do_pay.get("RtnCode") == 1:
                        print(f"    └─ [SUCCESS] 支付成功! 交易編號: {res_do_pay.get('EncData', {}).get('TradeNo')}")
                    else:
                        print(
                            f"    └─ [FAILED] 支付失敗! Code: {res_do_pay.get('RtnCode')} Msg: {res_do_pay.get('RtnMsg')}")
                else:
                    print(
                        f"    └─ [{i + 1}] Token 失敗! RtnCode: {res_qr.get('RtnCode')} | Msg: {res_qr.get('RtnMsg')}")
