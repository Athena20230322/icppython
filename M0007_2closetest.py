...
class CertificateApiClient:
    ...
    def refresh_login_token(self):
        ...
        try:
            ...
            print(f"無法從回應中提取 LoginTokenID: {e}")

        print("完整 API 回應: ", raw_response)
    ...
    def send_auth_sms(self):
        ...
        try:
            ...
            if not login_token_id:
                raise ValueError("logintokenid.txt 為空。")
            print(f"成功讀取 LoginTokenID: {login_token_id}")
        except FileNotFoundError:
            raise FileNotFoundError(f"找不到 Token 檔案: {login_token_file}，請先執行 refresh_login_token。")
        ...
        # 步驟 4: 處理回應，提取並儲存 AuthCode
        try:
            auth_code = decrypted_data.get("AuthCode")
            if auth_code is None:
                raise KeyError("回應中未找到 'AuthCode'。")
            ...
            with open(output_file_path, "w") as f:
                f.write(auth_code)
            print(f"AuthCode 已成功儲存至：{output_file_path}")

        except KeyError as e:
            print(f"無法從回應中提取 AuthCode: {e}")
...
if __name__ == '__main__':
    ...
    try:
        # ---
        # 執行第一步：取得並儲存 LoginTokenID
        # client.refresh_login_token()
        # ---

        # ---
        # 執行第二步：使用 LoginTokenID 發送簡訊
