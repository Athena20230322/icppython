...
class RsaCryptoHelper:
    ...
    def decrypt(self, enc_data):
        try:
            ...
            raise ValueError("解密失敗，請確認金鑰或資料。")
...
class CertificateApiClient:
    def __init__(self, base_url="https://icp-member-stage.icashpay.com.tw/"):
        ...
        self._aes_iv = None
        # 設定檔案路徑
        self.config_path = "C:\\icppython\\current_userop.json"
    ...
    def load_current_user(self):
        ...
        try:
            if os.path.exists(self.config_path):
                ...
                # 預設值，防止檔案不存在時當機
                return {"UserCode": "i1752714847", "CellPhone": "0950001621"}
    ...
    def _call_normal_api(self, action, payload):
        ...
        try:
            ...
            if login_token_id:
                ...
                print(f"Token 已儲存至：{self.token_file}")
        except Exception as e:
    ...
    def refresh_login_token(self):
        ...
        # 讀取當前切換的帳號資料
        user_info = self.load_current_user()
        print(f"正在為手機號碼 {user_info['CellPhone']} 取得 LoginToken...")

        url = "app/MemberInfo/RefreshLoginToken"
        request_payload = {
            'Timestamp': datetime.now().strftime("%Y/%m/%d %H:%M:%S"),
            'CellPhone': user_info['CellPhone']  # 從 0950001657 改為變數
        }
