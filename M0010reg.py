...
class CertificateApiClient:
    ...
    def CheckRegisterAuthSMS(self):
        """
        【修改重點】執行簡訊驗證碼的 API 呼叫。
        手機號碼、LoginTokenID 和 AuthCode 皆從檔案讀取。
        """
        self.generate_aes()
        ...
        # --- 【修改重點】從檔案讀取 AuthCode ---
        try:
            with open("C:\\icppython\\regauthcode.txt", 'r') as f:
                auth_code_from_file = f.read().strip()
        except FileNotFoundError:
            print("!!! 錯誤: 找不到驗證碼檔案 C:\\icppython\\regauthcode.txt !!!")
            raise
        # --- 修改結束 ---

        url = "app/MemberInfo/CheckRegisterAuthSMS"
        # --- 【修改重點】將讀取到的 AuthCode 放入 payload ---
        request_payload = {
            'Timestamp': datetime.now().strftime("%Y/%m/%d %H:%M:%S"),
            'CellPhone': cellphone_from_file,
            'LoginTokenID': token_from_file,
            'AuthCode': auth_code_from_file
        }
        # --- 修改結束 ---

        # --- 【修改重點】更新 print 敘述以反映真實情況 ---
        print(f"正在使用從檔案讀取的手機號碼 ({cellphone_from_file})、LoginTokenID ({token_from_file}) 及 AuthCode ({auth_code_from_file}) 來驗證簡訊")
        decrypted_response_str = self._call_normal_api(url, request_payload)

        # 因為此函式僅做驗證，不再需要將回傳的 AuthCode 寫入檔案，
        # 只需印出最終結果即可。
        print("\nCheckRegisterAuthSMS API 最終解密回應:")
...
if __name__ == '__main__':
    ...
    try:
        # --- 【修改重點】主程式現在呼叫 CheckRegisterAuthSMS ---
        client.CheckRegisterAuthSMS()
        print(f"\n======= 程式執行成功 =======")
