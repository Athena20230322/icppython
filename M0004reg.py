...
class CertificateApiClient:
    ...
    def _call_normal_api(self, action, payload):
        ...
        try:
            # 使用 separators 移除 JSON 字串中多餘的空格
            json_payload = json.dumps(payload, ensure_ascii=False, separators=(',', ':'))
    ...
    def AuthIDNO(self):
        ...
        url = "app/MemberInfo/AuthIDNO"

        # --- 從檔案讀取 LoginTokenID，以確保流程狀態一致 ---
        try:
        ...
        # --- 從檔案讀取 Idno，以確保使用的是同一個新註冊的用戶 ---
        try:
            with open("C:\\icppython\\last_idno.txt", 'r') as f:
                idno_from_file = f.read().strip()
        except FileNotFoundError:
            print("!!! 錯誤: 找不到 Idno 檔案 C:\\icppython\\last_idno.txt !!!")
            raise
        ...
        print(f"正在使用 LoginTokenID ({token_from_file}) 和身分證 ({idno_from_file}) 進行身分驗證 (AuthIDNO)")
