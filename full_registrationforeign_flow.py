...
# --- 共用函式 ---
def generate_foreigner_arc_id():
    """
    (***新增***)
    產生一個隨機的中華民國外來人口統一證號（新式）。
    格式為：區域碼(1碼英文字母) + 性別碼(8或9) + 流水號(7碼數字) + 檢查碼(1碼數字)。
    """
    letter_map = {
    ...
    gender = random.choice([8, 9])  # 外國人男性為8, 女性為9
...
class FullFlowApiClient:
    ...
    def run_full_flow(self):
        ...
        try:
            ...
            common_device_info = {
                "AppName": "002",
                "DeviceID": f"b4f194a{random.randint(100000, 999999)}",  # 隨機化DeviceID
                "DeviceInfo": f"Pixel 7_{random_suffix}",  # 隨機化DeviceInfo
                "IsSimulator": "0",
                "OS": "2"
            }
            print(
            ...
            # (***程式碼修改處***) 使用外國人居留證號產生器
            id_no = generate_foreigner_arc_id()
            ...
            log_data.update({
                ...
                'Idno': id_no, 'ConfirmSecPwd': confirm_sec_pwd, 'DeviceInfo': common_device_info
            })
            with open(LAST_IDNO_FILE, 'w') as f:
                f.write(id_no)
            print(f"動態資料已產生: UserCode={user_code}, CellPhone={cell_phone}, ARC No={id_no}")
            ...
            payload4 = {
                **common_device_info,
                ...
                "CName": "外國人一",
                "NationalityID": "1206",  # 範例國籍ID
                "Idno": id_no,  # (***程式碼修改處***) 使用產生的居留證號
                "EnName": "Johnbbb",
                "BirthDay": "1990-01-01",
                "UniformIssueDate": "2020-01-01",
                "UniformNumber": "F160000001",
                "UniformPermanentType": True,
                "fileCols": "img1,img2"
            }
            result4, _, _ = self._call_api("app/MemberInfo/AuthUniformID", payload4)
            reporter.add_step(current_step, "✅ 成功", payload4, result4)
            print("身分驗證成功。")

            # (***程式碼修改處***) 根據要求，註解掉步驟 5, 6, 7
            ...
            #     'CellPhone': cell_phone, 'Birthday': "20000101"
            # }
